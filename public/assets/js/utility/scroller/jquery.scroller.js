 ...
 ;(function ($, window) {
    ...
    var pub = {
        ...
        destroy: function() {
            return $(this).each(function(i, el) {
                ...
                if (data) {
                    data.$scroller.removeClass( [data.customClass, classes.base, classes.isActive].join(" ") ).attr("style", "");
            ...
            return $(this).each(function(i) {
                ...
                if (data) {
                    ...
                    if (data.horizontal) {
                        ...
                        // CUSTOM - added a check to see if parent is panel body
                        // so that we can determine if need to adjust scrollbar for padding
                        data.$scroller.css({
                            padding: 0
                        });
                        data.$content.css({
                        ...
                        data.handleBounds = {
                            top: 0,
                            bottom: data.frameHeight - data.handleHeight
                        };
    ...
    function build($scroller, opts) {
        if (!$scroller.hasClass(classes.base)) {
            ...
            // CUSTOM - added a check to see if parent is panel body
            // so that we can determine if need to adjust scrollbar for padding
            opts.paddingTop =    parseInt($scroller.css("padding-top"), 10);
            opts.paddingRight =  parseInt($scroller.css("padding-right"), 10);
            opts.paddingBottom = parseInt($scroller.css("padding-bottom"), 10);
            opts.paddingLeft =   parseInt($scroller.css("padding-left"), 10);
            // END CUSTOM - NOTE: left and right already existed
            // 
