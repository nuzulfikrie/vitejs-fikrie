...
(function($, window, document, undefined) {
    ...
    $.fn.adminpanel = function(options) {
        ...
        var defaults = {
            ...
            onDrop: function() {
                console.log('callback:', 'onDrop'); // Note: "onSave" is also fired if elements position changes after drop
            },
            onFinish: function() {
        ...
        plugin.on('click', '.panel-controls', function(e) {
            if (e.target.tagName.toLowerCase() == 'a') {
                e.preventDefault();

                // if a panel is being dragged disable menu clicks
                if ($('body.ui-drag-active').length) { return; }

                // determine appropriate event response
                methods.controlHandlers.call(e.target, options);
            }
        });
        ...
        var methods = {
            init: function(options) {
                ...
                // Helper function that adds unique ID's and filler panels 
                // to grid elements so that the space is preserved in empty columns
                $(pluginGrid).each(function(i, e) {
                    $(e).attr('id', 'grid-' + i);
                    if (!$(e).hasClass('sort-disable')) {
                        $(e).append('<div class="panel preserve-grid"></div>');
                    }
                });
                ...
                // Create controls for each admin panel
                ...
                // Init jQuery sortable on panels if settings is true. Sortable is a DOM
                // heavy event and timeout defers event and lets browser init when its ready
                if (dragSetting === true) { setTimeout(initSortable, 0); } // requestAnimationFrame(initSortable); }
                function initSortable() {
                    plugin.sortable({
                        items: $(pluginGrid).find('.panel:not(".sort-disable")'),
                        connectWith: '.admin-grid',
                        ...
                        create: function() {},
                        start: function(e, ui) {
                            // Add Body class to signal an active ui-drag
                            $('body').addClass('ui-drag-active');
                            ui.placeholder.height(ui.helper.outerHeight() - 4);
                        },
                        beforeStop: function() {
                            // onMove callback
                            if (typeof options.onDrop == 'function') { options.onDrop(); }
                        },
                        stop: function() {
                            // Remove Body class which signals active ui-drag
                            Body.removeClass('ui-drag-active');
                        },
                        update: function(event, ui) {
                            // Toggle Loading Indicator and store the position of the panels
            ...
            createControls: function(options) {
                ...
                // Library of available panel controls
                var Buttons = {
                ...
                // Loop through each admin panel and create a unique menu handler to open menu
                panels.each(function(i, e) {
                    var This = $(e);
                    var activeBtns = "";
                    var Data = This.data();

                    // Loop through each panel control menu and determine what 
                    // buttons should be visible based on the panels attributes/data    
                    $.each(Buttons, function(i, e) {
                        if (Data[i] == true) { activeBtns += e; } else if (Data[i] != false && !Data[i]) { activeBtns += e; } else if (Data[i] != false && i == "panelColor") { activeBtns += e; }
                    });

                    // Create panel menu container and add buttons and handler to container
                    var panelHeader = This.children('.panel-heading');
                    $('<span class="panel-controls"></span>').append(activeBtns).appendTo(panelHeader);
                    $('<div class="panel-controls-btn btn btn-default btn-gradient btn-lg"><span class="fa fa-bars"></span></div>').appendTo(panelHeader).on('click', function() {
                        panelHeader.toggleClass('panel-controls-open');
                    });
                });
            ...
            createMobileControls: function(options) {
                ...
                $.each(arr, function(i, e) {
                    ...
                    if (mobileSetting == true) {
                        ...
                        panelHeading.addClass('mobile-controls');

                        var timeoutID;
                        $('<div class="panel-controls-btn btn btn-default btn-gradient btn-lg"><span class="fa fa-bars"></span></div>').insertBefore(controls).on('click', function() {
                            panelHeading.toggleClass('panel-controls-open');
                        });
                    }
            ...
            controlHandlers: function(e) {
                ...
                var panelColor = function() {
                    ...
                    panelColorBox.on('click', '> span', function(e) {
                        ...
                        var altColors = 'panel-unstyled panel-primary panel-info panel-success panel-warning panel-danger panel-alert panel-system panel-dark panel-default panel-white';
                ...
                var panelFullscreen = function() {
                    ...
                    panelHeading.find('.panel-controls').removeClass('panel-controls-open');

                    // Remove fullscreen and 'esc' key handler
                    if (Body.hasClass('panel-fullscreen-active')) {
                        removeFullscreen();
                    }
                    // Convert to fullscreen and add new 'esc' key handler to allow removal via keyboard
                    else {
                        addFullscreen();
                    }
                    ...
                    function removeFullscreen() {
                        ...
                        if (dragSetting === true) { plugin.sortable("enable"); }
                        $(document).unbind('keydown.request_fullscreen');
                    }
                    ...
                    function addFullscreen() {
                        ...
                        if (dragSetting === true) { plugin.sortable("disable"); }
                        $(document).on('keydown.request_fullscreen', function(e) {
            ...
            applySettings: function(options) {
                ...
                $.each(parseSettings, function(i, e) {
                    ...
                    $.each(e, function(i, e) {
                        ...
                        if (panelColor) {
                            Target.removeClass(panelColors).addClass(panelColor).data('panel-color', panelColor);
                        }
                        if (panelCollapsed) {
                            Target.addClass('panel-collapsed').children('.panel-body, .panel-menu, .panel-footer').hide();
                        }
                        if (panelHidden) {
                            Target.removeClass('hidden').addClass('panel-hidden').hide();
                        }
                    });
                });
            },
