...
import ContentDataTable from '../ui/components/Modal/ContentDataTable';
interface Subtheme {
  name: string;
  key: string;
}
...
*/
  ...
  const confirmDialogMetadata = () => {
    ...
    axios
      .get(url)
      .then((response: { data: { status: string; data: any; }; }) => {
  ...
  const confirmTemplate = () => {
    ...
    let doiString = formik.values.doi ? formik.values.doi : '';
    confirmDialog({
      message: 'Are you sure you want to insert template? ' + doiString,
      header: 'Confirmation',
  ...
  const processMetadata = (metadata: any[], authorString: any, year: any,doi?:string) => {
    ...
    const inTextCitation = generateCitationUsingDoi(doi ?? '');
